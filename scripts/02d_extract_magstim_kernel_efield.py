...
from shamdose.magstim_kernel_subjects import (
    build_magstim_kernel_efield_for_cohort,
    print_magstim_kernel_cohort_summary,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Extract corrected Magstim sham E-field rows by applying the "
            "Smith-Peterchev empirical sham/active spatial kernel to each "
            "subject's active Magstim SimNIBS mesh."
        )
    )
    ...
    parser.add_argument(
        ...
        choices=[-1.0, 1.0],
        help=(
            "Sign of the empirical y-axis relative to the SimNIBS coil ydir. "
            "Use +1 for primary extraction; -1 for orientation sensitivity."
        ),
    )
    ...
    parser.add_argument(
        "--output-suffix",
        default="",
        help="Optional suffix for output files, e.g. TEST or yflip.",
    )
...
def main() -> None:
    ...
    print("\n[STEP 2d MAGSTIM EMPIRICAL-KERNEL SUBJECT EXTRACTION]")
    ...
    kernel_rows, qc = build_magstim_kernel_efield_for_cohort(
        step1=step1,
        metadata=metadata,
        cohort=args.cohort,
    ...
    out_rows = tables_dir / f"magstim_kernel_efield_{args.cohort}{suffix}.csv"
    out_qc = tables_dir / f"magstim_kernel_efield_qc_{args.cohort}{suffix}.csv"

    kernel_rows.to_csv(out_rows, index=False)
    qc.to_csv(out_qc, index=False)

    print("\n[OK] Saved corrected Magstim kernel E-field rows:")
    print(f"     {out_rows}")

    print("[OK] Saved corrected Magstim kernel QC table:")
    print(f"     {out_qc}")
